    ...
    def _generate_reference(self, agent_return, code, as_dict):
        ...
        for node_name in node_list:
            ...
            if as_dict:
                ...
                actions = agent_return.nodes[node_name]['detail'].actions
            if actions:
                ref_results = actions[0]['result'][0]['content'] if as_dict else actions[0].result[0]['content']
            if ref_results:
                try:
                    ...
                    logger.error(f"JSON decode error for node {node_name}: {e}")
                    logger.error(f"Raw ref_results: {ref_results}")
                    ref_results = None
                    ref2url = None
